...
@section('content')
    <main class="global-main">
        <div class="archive-section is-image">
            ...
            <h1 class="archive-title global-title is-tag">{{ $tag->name }}<span>{{ $tag->posts_count }}</span></h1>
            <p class="archive-excerpt global-excerpt">{{ $tag->meta_title }}</p>
        </div>
...
@push('scripts')
    ...
    <script>
        var ENDPOINT = "{{ route('showTagPosts', ['title' => $tag->name, 'id' => $tag->id]) }}";
        var page = 1;
